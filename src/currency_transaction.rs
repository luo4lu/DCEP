...
pub async fn digistal_transaction(
    ...
    //输出的数字货币
    let output_vec: Vec<DigitalCurrencyWrapper> =
        transaction_wrapper.gen_new_currency(&keypair_sm2, &mut rng);
    ...
    for output_id in output_currency_id.iter() {
        for input_id in input_currency_id.iter() {
            match transaction_sql
                .query(
                    "INSERT INTO currencys (output_id, input_id, cloud_user_id,
            create_time, update_time) VALUES ($1, $2, $3, now(), now())",
                    &[&output_id, &input_id, &head_str],
                )
                .await
            {
                Ok(_) => {
    ...
    match transaction_sql
        .query(
            "INSERT INTO exchanges (transaction_id, cloud_user_id, trans_info, trans_bin, amount, 
        create_time, update_time) VALUES ($1, $2, $3, $4, $5, now(), now())",
            &[
                &transaction_id,
                &head_str,
                &json_transaction,
                &req.curr_transaction,
                &total_amount,
            ],
        )
        .await
    {
        Ok(_) => {
            ...
            return HttpResponse::Ok().json(ResponseBody::<DcdsResponsetBody>::new_success(Some(
                DcdsResponsetBody {
